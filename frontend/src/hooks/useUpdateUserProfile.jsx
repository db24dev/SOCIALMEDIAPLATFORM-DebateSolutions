...
export const useUpdateCoverPhoto = () => {
    ...
    const { mutateAsync: updateCoverPhoto, isPending: isUpdatingCoverPhoto } = useMutation({
		mutationFn: async (img) => {
			try {
			  const res = await fetch(`/api/users/update`, {
				method: "POST",
				...
				body: JSON.stringify({coverImg: img}),
              });
			  const data = await res.json();
			  if (!res.ok) {
				throw new Error(data.error || "Something went wrong");
    ...
    return {updateCoverPhoto, isUpdatingCoverPhoto}
}
